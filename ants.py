...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        loc = self.place
        count = 0 
        while loc.entrance != None:
            if count > self.upper_bound:
                return None
            if not loc.is_hive:
                if len(loc.bees) > 0 and (count == self.lower_bound or count > self.lower_bound):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        for b in self.place.bees[:]: #when meeting bee
            b.reduce_health(amount)

        if self.health - amount <= 0: #when dead
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while pos:
            if pos.ant:
                if pos.ant.is_container:
                    pos.ant.double()
                    pos.ant.dub = True
                    if pos.ant.ant_contained:
                        pos.ant.ant_contained.double()
                        pos.ant.ant_contained.dub = True               
                else:
                    pos.ant.double()
                    pos.ant.dub = True
            pos = pos.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def action(gamestate):
            if target.slowed_counter > 0:
                if not gamestate.time % 2 != 0:
                    Bee.action(target, gamestate)
                target.slowed_counter = target.slowed_counter - 1
            else:
                Bee.action(target, gamestate)
        target.action = action
        target.slowed_counter = 5
        # END Problem EC
